...
def evaluate_welfare(envy_df, utilities_list, llm_allocation_indices, path):
    ...
    return welfare_df
# def generate_all_possible_allocations(num_agents: int, num_items: int) -> np.ndarray:
#     """
#     Generate all possible ways to allocate items to agents.
#     Returns a numpy array of shape (num_possible_allocations, num_agents, num_items)
#     where each allocation matrix has exactly one 1 in each column.
#     """
#     # Generate all possible assignments of items to agents
#     agents = range(num_agents)
#     possible_assignments = list(itertools.product(agents, repeat=num_items))
    
#     # Convert assignments to allocation matrices
#     num_allocations = len(possible_assignments)
#     allocation_matrices = np.zeros((num_allocations, num_agents, num_items))
...
# def find_envy_free_allocation(valuation_table: np.ndarray) -> tuple[np.ndarray, bool]:
#     """
#     Find an envy-free allocation if one exists.
    
#     Args:
#         valuation_table: numpy array of shape (num_agents, num_items) containing valuations
        
#     Returns:
#         tuple (allocation_matrix, found) where:
#         - allocation_matrix is the envy-free allocation if found (None if not found)
#         - found is True if an envy-free allocation was found, False otherwise
#     """
#     num_agents, num_items = valuation_table.shape
    
#     # Generate all possible allocation matrices
#     all_allocations = generate_all_possible_allocations(num_agents, num_items)
    
#     # Reshape valuation table to work with calculate_envy function
#     valuation_tables = np.expand_dims(valuation_table, axis=0)
    
#     # Check each allocation for envy-freeness
#     for i, allocation in enumerate(all_allocations):
#         allocation_expanded = np.expand_dims(allocation, axis=0)
#         envy_matrix_clipped, _ = calculate_envy(
#             valuation_tables, 
#             allocation_expanded,
#             num_agents,
#             num_items
...
#         # If maximum envy is 0, we found an envy-free allocation
#         if np.max(envy_matrix_clipped) == 0:
